...
if [ "$TIME_ZONE" ]; then
    ...
    echo "Container timezone set to: $TIME_ZONE"
fi
...
if [ ! -z "$SERVER_NAME" ]
    then
        ...
        echo "Changed server name to '$SERVER_NAME'..."
    else
...
# download latest version of churchcrm if not downloaded

if [ ! -f /web/html/index.php ]; then
  cd /tmp
  # Latest Release Including RC
  # Latest Full Release
  curl -s https://api.github.com/repos/churchcrm/CRM/releases/latest \
  ...
  sed -i "s/||DB_SERVER_PORT||/$MYSQL_DB_PORT/g" /web/html/Include/Config.php
...
if [ "$CHURCHCRM_BACKUP_LOCATION" ]; then
    if [ -d $CHURCHCRM_BACKUP_LOCATION ];
        then
            chmod 777 -R $CHURCHCRM_BACKUP_LOCATION
        else
            echo "Cannot find configured backup location."
    fi
fi
